	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < sortedArray.length - 1; i++) {
			...
			if (geoData[sortedArray[i]] != null) {
				...
				// Calculate the distance when the next schedule has GEO data
				// (for the last data)
				if (geoData[sortedArray[j]] != null) {
					distance[sortedArray[i]] = greatDistance.cal(lat[sortedArray[i]], lon[sortedArray[i]],
							lat[sortedArray[j]], lon[sortedArray[j]]);
				}
